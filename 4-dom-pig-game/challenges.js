...
var score, lastDice, roundScore, activePlayer, gamePlaying;
...
document.querySelector('.btn-roll').addEventListener('click', function() { // anonymous function - no name so cannot be called outside of this expression (created at runtime)
    if (gamePlaying) {
        ...
        // 3. coding challenge - player loses entire score if two 6s in a row rolled --OR-- update the round score IF the rolled number was NOT a 1
         if (dice === 6 && lastDice === 6) {
            ...
            prevRoll = dice; // store previous dice roll value in new variable
        } else {
            // next player
            nextPlayer();
        }
